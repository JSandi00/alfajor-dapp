...
import { CONTRACT_ADDRESS, CONTRACT_ABI, isAdmin } from '@/lib/contract';
...
export function ContractDashboard() {
  ...
  const handleSetTrialMinterConfig = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast('info', 'Trial minter config update submitted! Waiting for confirmation...');
    } catch (error: any) {
      const errorMessage = error?.message || 'Configuration update failed';
      showToast('error', `Failed to update trial minter config: ${errorMessage}`);
    }
  };
  ...
  const handleSetTrialBurnerConfig = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast('info', 'Trial burner config update submitted! Waiting for confirmation...');
    } catch (error: any) {
      const errorMessage = error?.message || 'Configuration update failed';
      showToast('error', `Failed to update trial burner config: ${errorMessage}`);
    }
  };
  ...
  useEffect(() => {
    if (trialMinterConfigTxSuccess) {
      showToast('success', 'Trial minter configuration updated successfully!');
      // Clear form
  ...
  useEffect(() => {
    if (trialBurnerConfigTxSuccess) {
      showToast('success', 'Trial burner configuration updated successfully!');
      // Clear form
  ...
  return (
    ...
    <div className="space-y-6">
      ...
      {isAdmin(address) && (
        <div className="bg-gradient-to-r from-purple-500/10 to-pink-500/10 backdrop-blur-md rounded-xl p-6 border border-purple-400/30">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-xl font-semibold text-white flex items-center">
              <svg className="w-6 h-6 text-purple-400 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
